#!/usr/bin/env python3
"""
CONTINUOUS TRAINING + CAPTAIN EVALUATION

Trains model on synthetic data and evaluates performance via captain agents.
Reports captain_score (ModelGuided / Random) and oracle_fraction (ModelGuided / Oracle).
"""
import torch
...
print("=" * 70)
print("CONTINUOUS TRAINING + CAPTAIN AGENT EVALUATION")
print("=" * 70)
...
# Checkpoint dir
checkpoint_dir = Path("checkpoints_eval")
...
for epoch in range(1, num_epochs + 1):
    ...
    if epoch % eval_interval == 0:
        ...
        # Run evaluation
        print(f"  Running 5 episodes per captain (60s each)...")
        report = evaluate_model(
            model_path=str(ckpt_path),
            n_episodes=5,
            duration_s=60,
            seeds=[epoch, epoch+1, epoch+2, epoch+3, epoch+4],  # vary seeds
        )
...
print(f"Best checkpoint: {checkpoint_dir}/best_by_captain_score.pt")
